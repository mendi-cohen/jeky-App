 ...
 import { useReservation } from '../Contexts/Success';
  const PlaceReservationForm = ({ object }) => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API}/users/addUser`, {
          method: 'POST',
          body: formData,  
        });
    ...
    const filteredSeats = seats.filter(seat =>
      seat.color === 'green' && seat.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
          ...
          <div className="mb-4">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {selectedSeats.map((seatId) => {
                const seat = seats.find(s => s.id === seatId); // מציאת הכיסא לפי ה-ID
                return (
                  <div key={seatId} className="flex items-center bg-blue-200 text-blue-700 px-2 py-1 rounded mb-1">
                    <span>{seat ? seat.name : 'כיסא לא נמצא'}</span> {/* הצגת השם של הכיסא */}
                    <button
                      onClick={() => handleRemoveSeat(seatId)} // מסירת ה-ID של הכיסא
